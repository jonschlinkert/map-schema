...
Field.prototype.get = function(prop) {
  return koalas(utils.get(this, prop), utils.get(this.config, prop));
};
...
Field.prototype.normalize = function(val, key, config, schema) {
  if (typeof this.config.normalize === 'function') {
    var ctx = schema || this;
    val = this.config.normalize.apply(ctx, arguments);
...
/**
 * Returns true if `field` is an instance of `Field`.
 *
 * @param {Object} `field`
 * @return {Boolean}
 * @api public
